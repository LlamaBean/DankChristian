...
_allowed_styles = ["color", "font-weight", "margin-bottom"]
...
_clean_w_links = bleach.Cleaner(
    ...
    filters=[
        partial(
            LinkifyFilter, skip_tags=["pre"], parse_email=False, callbacks=[a_modify]
        )
    ],
)
...
def sanitize(text, linkgen=False, flair=False):
    ...
    if (
        start in sanitized
        and end in sanitized
        and start in sanitized.split(end)[0]
        and end in sanitized.split(start)[1]
    ):
        sanitized = sanitized.replace(start, '<span class="spoiler">').replace(
            end, "</span>"
        )
    ...
    for i in re.finditer(":(.{1,30}?):", sanitized):
        if path.isfile(f"./files/assets/images/emojis/{i.group(1)}.gif"):
            sanitized = sanitized.replace(
                f":{i.group(1)}:",
                f'<img data-toggle="tooltip" title="{i.group(1)}" delay="0" height={emojisize} src="https://{site}/assets/images/emojis/{i.group(1)}.gif"<span>',
            )

    sanitized = (
        sanitized.replace("https://www.", "https://")
        .replace("https://youtu.be/", "https://youtube.com/embed/")
        .replace("https://music.youtube.com/watch?v=", "https://youtube.com/embed/")
        .replace("/watch?v=", "/embed/")
        .replace("https://open.spotify.com/", "https://open.spotify.com/embed/")
        .replace("https://streamable.com/", "https://streamable.com/e/")
        .replace("https://youtube.com/shorts/", "https://youtube.com/embed/")
    )

    for i in re.finditer(
        '<a href="(https://(streamable|youtube).com/(e|embed)/.*?)"', sanitized
    ):
        url = i.group(1)

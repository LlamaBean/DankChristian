...
def get_user(username, v=None, graceful=False):
    ...
    user = (
        g.db.query(User)
        .filter(
            or_(User.username.ilike(username), User.original_username.ilike(username))
        )
        .first()
    )
    ...
    if v:
        block = (
            ...
            .filter(
                or_(
                    and_(UserBlock.user_id == v.id, UserBlock.target_id == user.id),
                    and_(UserBlock.user_id == user.id, UserBlock.target_id == v.id),
                )
            )
            .first()
        )
...
def get_post(i, v=None, graceful=False, **kwargs):
    ...
    if v:
        ...
        items = g.db.query(Submission).filter(Submission.id == i).first()
        if not items and not graceful:
            abort(404)
        x = items
...
def get_posts(pids, v=None):
    ...
    if v:
        vt = (
            g.db.query(Vote)
            .filter(Vote.submission_id.in_(pids), Vote.user_id == v.id)
            .subquery()
        )
...
def get_comment(i, v=None, graceful=False, **kwargs):
    ...
    if v:
        ...
        if not comment and not graceful:
            abort(404)
        ...
        vt = (
            ...
            .filter_by(user_id=v.id, comment_id=Comment.id)
            .first()
        )
...
def get_comments(cids, v=None):
    ...
    if v:
        ...
        comments = (
            comments.join(votes, votes.c.comment_id == Comment.id, isouter=True)
...
def get_domain(s):
    ...
    doms = [
        ...
        .filter(BannedDomain.domain.in_(domain_list))
        .all()
    ]

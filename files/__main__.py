...
app.config["DATABASE_URL"] = environ.get(
    "DATABASE_CONNECTION_POOL_URL", environ.get("DATABASE_URL")
)
...
app.config["FORCE_HTTPS"] = (
    int(environ.get("FORCE_HTTPS", 1))
    if (
        "localhost" not in app.config["SERVER_NAME"]
        and "127.0.0.1" not in app.config["SERVER_NAME"]
    )
    else 0
)
...
app.config["SPAM_SIMILARITY_THRESHOLD"] = float(
    environ.get("SPAM_SIMILARITY_THRESHOLD")
)
app.config["SPAM_SIMILAR_COUNT_THRESHOLD"] = int(
    environ.get("SPAM_SIMILAR_COUNT_THRESHOLD")
)
app.config["SPAM_URL_SIMILARITY_THRESHOLD"] = float(
    environ.get("SPAM_URL_SIMILARITY_THRESHOLD")
)
app.config["COMMENT_SPAM_SIMILAR_THRESHOLD"] = float(
    environ.get("COMMENT_SPAM_SIMILAR_THRESHOLD")
)
app.config["COMMENT_SPAM_COUNT_THRESHOLD"] = int(
    environ.get("COMMENT_SPAM_COUNT_THRESHOLD")
)
...
app.config["CACHE_OPTIONS"] = (
    {"connection_pool": redispool} if app.config["CACHE_TYPE"] == "redis" else {}
)
...
def get_useragent_ban_response(user_agent_str):
    ...
    result = (
        g.db.query(files.classes.Agent)
        .filter(files.classes.Agent.kwd.in_(user_agent_str.split()))
        .first()
    )
    if result:
        return True, (
            result.mock or "Follow the robots.txt, dumbass",
...
def before_request():
    ...
    if request.method.lower() != "get" and app.config["READ_ONLY"]:
        return {
            "error": f"{app.config['SITE_NAME']} is currently in read-only mode."
        }, 500
    ...
    if (
        ...
        and "localhost" not in app.config["SERVER_NAME"]
    ):

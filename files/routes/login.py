...
def login_get(v):
    ...
    return render_template(
        "login.html", failed=False, i=random_background(), redirect=redir
    )
...
def login_post():
    ...
    if "@" in username:
        account = g.db.query(User).filter(User.email.ilike(username)).first()
    else:
    ...
    if request.form.get("password"):
        ...
        if not validate_hash(
            f"{account.id}+{request.form.get('time')}+2fachallenge", formhash
        ):
            return redirect("/login")
...
def sign_up_get(v):
    ...
    if ref_user and (ref_user.id in session.get("history", [])):
        return render_template("sign_up_failed_ref.html", i=random_image())
    ...
    formkey = hmac.new(
        key=bytes(environ.get("MASTER_KEY"), "utf-16"),
...
def sign_up_post(v):
    ...
    def new_signup(error):
        ...
        if request.form.get("referred_by"):
            user = (
                g.db.query(User).filter_by(id=request.form.get("referred_by")).first()
            )
            if user:
    ...
    if existing_account or (
        email and g.db.query(User).filter(User.email.ilike(email)).first()
    ):
        ...
        new_user = User(
            ...
            ban_evade=int(
                any(
                    [
                        x.is_banned
                        for x in g.db.query(User)
                        .filter(User.id.in_(tuple(session.get("history", []))))
                        .all()
                        if x
                    ]
                )
    ...
    # give an alpha badge
    alpha_badge = Badge(user_id=new_user.id, badge_id=6)

    g.db.add(alpha_badge)
    ...
    send_notification(
        1,
        new_user,
        "Welcome to DankChristian! A place for Christian memes and uncensored discussion.",
    )
...
def get_forgot():
    ...
    return render_template("forgot_password.html", i=random_image())
...
def lost_2fa(v):
    ...
    return render_template("lost_2fa.html", i=random_image(), v=v)
...
def request_2fa_disable():
    ...
    valid = int(time.time())
    token = generate_hash(
        f"{user.id}+{user.username}+disable2fa+{valid}+{user.mfa_secret}+{user.login_nonce}"
    )
    ...
    send_mail(
        ...
        html=render_template("email/2fa_remove.html", action_url=action_url, v=user),
    )
...
def reset_2fa():
    ...
    if now > t + 3600 * 24:
        return render_template(
            "message.html", title="Expired Link", error="That link has expired."
        )

...
def post_pid_comment_cid(cid, pid=None, anything=None, v=None):
    ...
    if not comment.parent_submission and not (v and v.admin_level == 6):
        abort(403)

    if not pid:
        if comment.parent_submission:
            pid = comment.parent_submission
        else:
            pid = 6489
    ...
    if (
        ...
        if request.headers.get("Authorization"):
            return {
                "error": f"This content is not suitable for some users and situations."
            }
        else:
            render_template("errors/nsfw.html", v=v)
    ...
    for i in range(200 - context):
        if v:
            ...
            comments = (
                comments.filter(Comment.parent_comment_id.in_(current_ids))
            ...
            if sort == "top":
                comments = sorted(
                    comments.all(), key=lambda x: x[0].score, reverse=True
                )
            elif sort == "bottom":
    ...
    if request.headers.get("Authorization"):
        return top_comment.json
    else:
        return post.rendered_page(
            v=v, sort=sort, comment=top_comment, comment_info=comment_info
        )
...
def api_comment(v):
    ...
    existing = (
        ...
        .options(contains_eager(Comment.comment_aux))
        .first()
    )
    if existing:
        return (
            jsonify({"error": f"You already made that comment: {existing.permalink}"}),
            409,
        )
    ...
    if parent.author.any_block_exists(v) and not v.admin_level >= 3:
        return (
            jsonify(
                {
                    "error": "You can't reply to users who have blocked you, or users you have blocked."
                }
            ),
            403,
        )
    ...
    if not is_bot:
        ...
        similar_comments = (
            ...
            .filter(
                ...
                < app.config["COMMENT_SPAM_SIMILAR_THRESHOLD"],
    ...
    for link in links:
        ...
        if badlink:
            return (
                jsonify(
                    {
                        "error": f"Remove the following link and try again: `{check_url}`. Reason: {badlink.reason}"
                    }
                ),
                403,
            )
    ...
    if not v.shadowbanned:
        ...
        for x in (
            ...
            .filter_by(submission_id=c.parent_submission)
            .all()
        ):
            notify_users.add(x)
    ...
    v.comment_count = (
        v.comments.filter(Comment.parent_submission != None)
...
def edit_comment(cid, v):
    ...
    with CustomRenderer(post_id=c.post.id) as renderer:
        body_md = renderer.render(mistletoe.Document(body))
    body_html = sanitize(body_md, linkgen=True)
...
def unsave_comment(cid, v):
    ...
    save = (
        ...
        .filter_by(user_id=v.id, submission_id=comment.id, type=2)
        .first()
    )

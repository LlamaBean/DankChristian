...
def unsubscribe(v, post_id):
    sub = (
        g.db.query(Subscription).filter_by(user_id=v.id, submission_id=post_id).first()
    )
    g.db.delete(sub)
    return "", 204
...
def u_username(username, v=None):
    ...
    if v and u.id != v.id:
        ...
        if view:
            ...
            view = ViewerRelationship(user_id=u.id, viewer_id=v.id)
...
def follow_user(username, v):
    ...
    target.stored_subscriber_count = (
        g.db.query(Follow).filter_by(target_id=target.id).count()
    )
    g.db.add(target)

    existing = (
        g.db.query(Notification).filter_by(followsender=v.id, user_id=target.id).first()
    )
    if not existing:
        send_follow_notif(v.id, target.id, f"@{v.username} has followed you!")
    return "", 204
...
def unfollow_user(username, v):
    ...
    if not existing:
        send_unfollow_notif(v.id, target.id, f"@{v.username} has unfollowed you!")
    return "", 204

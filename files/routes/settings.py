...
def settings_profile_post(v):
    ...
    if request.values.get("animatedname", v.animatedname) != v.animatedname:
        if v.animatedname == False:
            ...
            if v.id not in users:
                return {
                    "error": "You must be in the top 25 leaderboard or be a patron to apply an animated name!"
                }, 403
        updated = True
    ...
    if request.values.get("bio"):
        ...
        if bio == v.bio:
            return render_template(
                "settings_profile.html", v=v, error="You didn't change anything"
            )

        for i in re.finditer(
            "^(https:\/\/.*\.(png|jpg|jpeg|gif|PNG|JPG|JPEG|GIF))", bio, re.MULTILINE
        ):
            bio = bio.replace(i.group(1), f"![]({i.group(1)})")
        bio = (
            bio.replace("\n", "\n\n")
            .replace("\n\n\n\n\n\n", "\n\n")
            .replace("\n\n\n\n", "\n\n")
        ...
        g.db.add(v)
        return render_template(
            "settings_profile.html", v=v, msg="Your bio has been updated."
        )
    ...
    if request.values.get("filters"):
        ...
        g.db.add(v)
        return render_template(
            "settings_profile.html", v=v, msg="Your custom filters have been updated."
        )

    defaultsortingcomments = request.values.get("defaultsortingcomments")
    if defaultsortingcomments:
        if defaultsortingcomments in [
            "new",
            "old",
            "controversial",
            "top",
...
def settings_security_post(v):
    if request.form.get("new_password"):
        if request.form.get("new_password") != request.form.get("cnf_password"):
            return redirect(
                "/settings/security?error=" + escape("Passwords do not match.")
            )
        ...
        return redirect(
            "/settings/security?msg=" + escape("Your password has been changed.")
        )
    ...
    if request.form.get("new_email"):
        ...
        if new_email == v.email:
            return redirect(
                "/settings/security?error=" + escape("That email is already yours!")
            )

        # check to see if email is in use
        existing = (
            g.db.query(User)
            .filter(User.id != v.id, func.lower(User.email) == new_email.lower())
            .first()
        )
        if existing:
            return redirect(
    ...
    if request.form.get("2fa_token", ""):
        ...
        if not v.verifyPass(request.form.get("password")):
            return redirect(
                "/settings/security?error=" + escape("Invalid password or token.")
            )
        ...
        return redirect(
            "/settings/security?msg=" + escape("Two-factor authentication enabled.")
        )
    ...
    if request.form.get("2fa_remove", ""):
        ...
        return redirect(
            "/settings/security?msg=" + escape("Two-factor authentication disabled.")
        )
...
def settings_log_out_others(v):
    ...
    if not v.verifyPass(submitted_password):
        return (
            render_template("settings_security.html", v=v, error="Incorrect Password"),
            401,
        )
    ...
    return render_template(
        "settings_security.html", v=v, msg="All other devices have been logged out"
    )
...
def settings_images_profile(v):
    ...
    if request.headers.get("cf-ipcountry") == "T1":
        return "Image uploads are not allowed through TOR.", 403
    ...
    return render_template(
        "settings_profile.html", v=v, msg="Profile picture successfully updated."
    )
...
def settings_images_banner(v):
    ...
    return render_template(
        "settings_profile.html", v=v, msg="Banner successfully updated."
    )
...
def settings_delete_profile(v):
    ...
    g.db.add(v)
    return render_template(
        "settings_profile.html", v=v, msg="Profile picture successfully removed."
    )
...
def settings_delete_banner(v):
    ...
    return render_template(
        "settings_profile.html", v=v, msg="Banner successfully removed."
    )
...
def settings_block_user(v):
    ...
    existing = (
        g.db.query(Notification).filter_by(blocksender=v.id, user_id=user.id).first()
    )
    if not existing:
        send_block_notif(v.id, user.id, f"@{v.username} has blocked you!")
...
def settings_unblock_user(v):
    ...
    existing = (
        g.db.query(Notification).filter_by(unblocksender=v.id, user_id=user.id).first()
    )
    if not existing:
        send_unblock_notif(v.id, user.id, f"@{v.username} has unblocked you!")
...
def settings_remove_discord(v):
    ...
    if v.admin_level > 1:
        return render_template(
            "settings_filters.html", v=v, error="Admins can't disconnect Discord."
        )
...
def settings_name_change(v):
    ...
    if not re.match(valid_username_regex, new_name):
        return render_template(
            "settings_profile.html", v=v, error=f"This isn't a valid username."
        )
    ...
    x = (
        g.db.query(User)
        .options(lazyload("*"))
        .filter(or_(User.username.ilike(name), User.original_username.ilike(name)))
        .first()
    )
...
def settings_song_change(v):
    ...
    if (
        song == ""
        and v.song
        and path.isfile(f"/songs/{v.song}.mp3")
        and g.db.query(User).filter_by(song=v.song).count() == 1
    ):
    ...
    if song.startswith(
        ...
        return render_template(
            "settings_profile.html", v=v, error=f"Not a youtube link."
        )
    ...
    req = requests.get(
        f"https://www.googleapis.com/youtube/v3/videos?id={id}&key={YOUTUBE_KEY}&part=contentDetails"
    ).json()
    try:
...
def settings_title_change(v):
    ...
    if not re.match(valid_title_regex, new_name):
        return render_template(
            "settings_profile.html", v=v, error=f"This isn't a valid flair."
        )

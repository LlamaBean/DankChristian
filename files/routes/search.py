...
valid_params = ["author", "domain", "over18"]
...
def searchcommentlisting(criteria, v=None, page=1, t="None", sort="top"):
    ...
    if not (v and v.admin_level >= 3):
        comments = comments.filter(Comment.deleted_utc == 0, Comment.is_banned == False)
...
def searchposts(v):
    ...
    if request.headers.get("Authorization"):
        return {"data": [x.json for x in posts]}
    else:
...
def searchcomments(v):
    ...
    if request.headers.get("Authorization"):
        return [x.json for x in comments]
    else:
        return render_template(
...
def searchusers(v):
    ...
    users = users.order_by(
        User.username.ilike(term).desc(), User.stored_subscriber_count.desc()
    )

...
def post_id(pid, anything=None, v=None):
    ...
    if v:
        ...
        comments = (
            comments.filter(Comment.parent_submission == post.id)
            .join(votes, votes.c.comment_id == Comment.id, isouter=True)
            .join(blocking, blocking.c.target_id == Comment.author_id, isouter=True)
            .join(blocked, blocked.c.user_id == Comment.author_id, isouter=True)
        )
        ...
        if sort == "top":
            ...
            comments = sorted(
                comments.all(), key=lambda x: x[0].score_disputed, reverse=True
            )
        elif sort == "random":
            c = comments.all()
        ...
        for c in comments:
            ...
            if (
                comment.author
                and comment.author.shadowbanned
                and not (v and v.id == comment.author_id)
            ):
                continue
        ...
        comments = g.db.query(Comment).filter(Comment.parent_submission == post.id)
        ...
        if sort == "top":
            ...
            comments = sorted(
                comments.all(), key=lambda x: x.score_disputed, reverse=True
            )
        elif sort == "random":
            c = comments.all()
            comments = random.sample(c, k=len(c))
            ...
            for comment in comments:
                if comment.author and comment.author.shadowbanned:
                    ...
                    try:
                        ...
                        g.db.rollback()
                    comment.upvotes = (
                        g.db.query(CommentVote)
                        .filter_by(comment_id=comment.id, vote_type=1)
                        .count()
                    )
                    g.db.add(comment)
    ...
    g.db.add(post)
    g.db.commit()
    if isinstance(session.get("over_18", 0), dict):
        session["over_18"] = 0
    if (
        post.over_18
        and not (v and v.over_18)
        and not session.get("over_18", 0) >= int(time.time())
    ):
    ...
    if request.headers.get("Authorization"):
        ...
        return post.rendered_page(v=v, sort=sort)
...
def edit_post(pid, v):
    ...
    for i in re.finditer(
        "^(https:\/\/.*\.(png|jpg|jpeg|gif|PNG|JPG|JPEG|GIF))", body, re.MULTILINE
    ):
    ...
    for link in links:
        ...
        badlink = (
            g.db.query(BadLink)
            .filter(literal(check_url).contains(BadLink.link))
            .first()
        )
        if badlink:
            if badlink.autoban:
    ...
    if v.agendaposter and "trans lives matter" not in body_html.lower():
        ...
        c_aux = CommentAux(id=c_jannied.id, body_html=body_jannied_html, body=body)
        g.db.add(c_aux)
        g.db.flush()
    ...
    for x in notify_users:
        send_notification(
            1, x, f"@{v.username} has mentioned you: https://{site}{p.permalink}"
        )
...
def get_post_title(v):
    ...
    headers = {
        "User-Agent": f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.72 Safari/537.36"
    }
    try:
    ...
    try:
        ...
        data = {"url": url, "title": soup.find("title").string}
...
def thumbs(new_post):
    ...
    if x.headers.get("Content-Type", "").startswith("text/html"):
        ...
        meta_tags = ["twitter:image", "og:image", "thumbnail"]
        ...
        return (
            False,
            f'Unknown content type {x.headers.get("Content-Type")} for submitted content',
        )
...
def submit_post(v):
    ...
    if not title:
        ...
        if request.headers.get("Authorization"):
            return {"error": "500 character limit for titles"}, 400
        else:
            render_template(
                "submit.html",
                v=v,
    ...
    if (
        ...
        and not request.form.get("body")
        and not request.files.get("file", None)
    ):
    ...
    if "twitter.com" in domain:
        ...
        if not yt_id or len(yt_id) != 11:
            ...
            if t:
                embed = f"https://youtube.com/embed/{yt_id}?start={t}"
            else:
                embed = f"https://youtube.com/embed/{yt_id}"
        ...
        embed = requests.get(
            ...
            params={
                "url": url,
                "access_token": environ.get("FACEBOOK_TOKEN", "").strip(),
                "omitscript": "true",
            },
        ...
        try:
            matches = re.match(
                re.compile(f"^.*{domain}/post/+\w+/(\w+)(/\w+/(\w+))?"), url
            )
            post_id = matches.group(1)
            comment_id = matches.group(3)
            if comment_id:
                embed = (
                    f"https://{app.config['SERVER_NAME']}/embed/comment/{comment_id}"
                )
            else:
                embed = f"https://{app.config['SERVER_NAME']}/embed/post/{post_id}"
        except:
            embed = None
    ...
    if max(len(similar_urls), len(similar_posts)) >= threshold:
        ...
        for post in similar_posts + similar_urls:
            ...
            g.db.add(post)
            ma = ModAction(
                user_id=23, target_submission_id=post.id, kind="ban_post", note="spam"
            )
            g.db.add(ma)
        g.db.commit()
        return redirect("/notifications")
    ...
    for x in notify_users:
        send_notification(
            1, x, f"@{v.username} has mentioned you: https://{site}{new_post.permalink}"
        )
    ...
    if not new_post.private:
        for follow in v.followers:
            ...
            send_notification(
                23,
                user,
                f"@{v.username} has made a new post: [{title}](https://{site}{new_post.permalink})",
            )
...
def toggle_comment_nsfw(cid, v):
    ...
    if not comment.author_id == v.id and not v.admin_level >= 3:
        abort(403)
...
def unsave_post(pid, v):
    ...
    save = (
        g.db.query(SaveRelationship)
        .filter_by(user_id=v.id, submission_id=post.id, type=1)
        .first()
    )

...
def flagged_posts(v):
    ...
    posts = (
        ...
        .filter_by(is_approved=0, is_banned=False)
...
def flagged_comments(v):
    ...
    posts = (
        ...
        .filter_by(is_approved=0, is_banned=False)
        .join(Comment.flags)
        .options(contains_eager(Comment.flags))
...
def badge_grant_get(v):
    ...
    errors = {
        "already_owned": "That user already has that badge.",
        "no_user": "That user doesn't exist.",
    }

    return render_template(
        "admin/badge_grant.html",
        v=v,
        badge_types=badge_types,
        error=errors.get(request.args.get("error"), None)
        ...
        if len(grant_awards):
            ...
            for name in grant_awards:
                for count in range(grant_awards[name]):
                    ...
                    _awards.append(
                        AwardRelationship(id=thing, user_id=user.id, kind=name)
                    )
...
def participation_stats(v):
    ...
    data = {
        "valid_users": g.db.query(User).count(),
        ...
        "listed_posts": g.db.query(Submission)
        ...
        "deleted_posts": g.db.query(Submission)
        .filter(Submission.deleted_utc > 0)
        .count(),
        "posts_last_24h": g.db.query(Submission)
        .filter(Submission.created_utc > day)
        .count(),
        ...
        "comments_last_24h": g.db.query(Comment)
        .filter(Comment.created_utc > day)
        .count(),
        ...
        "awards_given": g.db.query(AwardRelationship)
        .filter(
            or_(
    ...
    return render_template(
        "admin/content_stats.html", v=v, title="Content Statistics", data=data
    )
...
def alt_votes_get(v):
    ...
    u1_post_ups = (
        g.db.query(Vote.submission_id).filter_by(user_id=u1.id, vote_type=1).all()
    )
    u1_post_downs = (
        g.db.query(Vote.submission_id).filter_by(user_id=u1.id, vote_type=-1).all()
    )
    u1_comment_ups = (
        g.db.query(CommentVote.comment_id).filter_by(user_id=u1.id, vote_type=1).all()
    )
    u1_comment_downs = (
        g.db.query(CommentVote.comment_id).filter_by(user_id=u1.id, vote_type=-1).all()
    )
    u2_post_ups = (
        g.db.query(Vote.submission_id).filter_by(user_id=u2.id, vote_type=1).all()
    )
    u2_post_downs = (
        g.db.query(Vote.submission_id).filter_by(user_id=u2.id, vote_type=-1).all()
    )
    u2_comment_ups = (
        g.db.query(CommentVote.comment_id).filter_by(user_id=u2.id, vote_type=1).all()
    )
    u2_comment_downs = (
        g.db.query(CommentVote.comment_id).filter_by(user_id=u2.id, vote_type=-1).all()
    )
    ...
    data["u1_post_ups_unique"] = (
        100 * data["u1_only_post_ups"] // len(u1_post_ups) if u1_post_ups else 0
    )
    data["u2_post_ups_unique"] = (
        100 * data["u2_only_post_ups"] // len(u2_post_ups) if u2_post_ups else 0
    )
    data["u1_post_downs_unique"] = (
        100 * data["u1_only_post_downs"] // len(u1_post_downs) if u1_post_downs else 0
    )
    data["u2_post_downs_unique"] = (
        100 * data["u2_only_post_downs"] // len(u2_post_downs) if u2_post_downs else 0
    )

    data["u1_comment_ups_unique"] = (
        100 * data["u1_only_comment_ups"] // len(u1_comment_ups)
        if u1_comment_ups
        else 0
    )
    data["u2_comment_ups_unique"] = (
        100 * data["u2_only_comment_ups"] // len(u2_comment_ups)
        if u2_comment_ups
        else 0
    )
    data["u1_comment_downs_unique"] = (
        100 * data["u1_only_comment_downs"] // len(u1_comment_downs)
        if u1_comment_downs
        else 0
    )
    data["u2_comment_downs_unique"] = (
        100 * data["u2_only_comment_downs"] // len(u2_comment_downs)
        if u2_comment_downs
        else 0
    )

    return render_template("admin/alt_votes.html", u1=u1, u2=u2, v=v, data=data)
...
def admin_link_accounts(v):
    ...
    new_alt = Alt(user1=u1, user2=u2, is_manual=True)

    g.db.add(new_alt)
    g.db.commit()

    return redirect(
        f"/admin/alt_votes?u1={g.db.query(User).get(u1).username}&u2={g.db.query(User).get(u2).username}"
    )
...
def admin_removed(v):
    ...
    ids = (
        g.db.query(Submission.id)
        .options(lazyload("*"))
        .filter_by(is_banned=True)
        .order_by(Submission.id.desc())
        .offset(25 * (page - 1))
        .limit(26)
        .all()
...
def admin_image_purge(v):
    ...
    if image:
        requests.delete(
            f"https://api.imgur.com/3/image/{image.deletehash}",
...
def admin_image_ban(v):
    ...
    badpic = g.db.query(BadPic).filter_by(phash=h).first()
...
def agendaposter(user_id, v):
    ...
    if not user.agendaposter:
        kind = "unagendaposter"
    else:
        kind = "agendaposter"
        note = f"for {request.form.get('days')} days" if expiry else "never expires"

    ma = ModAction(kind=kind, user_id=v.id, target_user_id=user.id, note=note)
    g.db.add(ma)
...
def admin_title_change(user_id, v):
    ...
    if user.flairchanged:
        ...
        kind = "set_flair_notlocked"

    ma = ModAction(
        kind=kind, user_id=v.id, target_user_id=user.id, note=f'"{new_name}"'
    )
    g.db.add(ma)
...
def admin_distinguish_comment(c_id, v):
    ...
    html = str(
        BeautifulSoup(html, features="html.parser").find(
            id=f"comment-{comment.id}-only"
        )
    )
...
def refund(v):
    for u in g.db.query(User).all():
        ...
        posts = sum(
            [
                x[0] + x[1] - 1
                for x in g.db.query(Submission.upvotes)
                .options(lazyload("*"))
                .filter_by(author_id=u.id, is_banned=False, deleted_utc=0)
                .all()
            ]
        )
        comments = sum(
            [
                x[0] + x[1] - 1
                for x in g.db.query(Comment.upvotes)
                .options(lazyload("*"))
                .filter_by(author_id=u.id, is_banned=False, deleted_utc=0)
                .all()
            ]
...
def admin_banned_domains(v):
    ...
    return render_template(
        "admin/banned_domains.html", v=v, banned_domains=banned_domains
    )
...
def user_stat_data(v):
    ...
    midnight_this_morning = time.struct_time(
        (now.tm_year, now.tm_mon, now.tm_mday, 0, 0, 0, now.tm_wday, now.tm_yday, 0)
    )
    ...
    daily_signups = [
        {
            "date": time.strftime("%d", time.gmtime(day_cutoffs[i + 1])),
            "day_start": day_cutoffs[i + 1],
            "signups": g.db.query(User)
            .filter(
    ...
    user_stats = {
        "current_users": g.db.query(User).filter_by(is_banned=0, reserved=None).count(),
        "banned_users": g.db.query(User).filter(User.is_banned != 0).count(),
        "reserved_users": g.db.query(User).filter(User.reserved != None).count(),
        "email_verified_users": g.db.query(User)
        .filter_by(is_banned=0, is_activated=True)
        .count(),
    }
    ...
    x = create_plot(
        sign_ups={"daily_signups": daily_signups},
...
def create_plot(**kwargs):
    ...
    multi_plots = multiple_plots(
        sign_ups=daily_signups,
        posts=post_stats,
...
def multiple_plots(**kwargs):
    ...
    signup_chart.plot(kwargs["daily_times"][::-1], kwargs["sign_ups"], color="red")
    posts_chart.plot(kwargs["daily_times"][::-1], kwargs["posts"], color="green")
    comments_chart.plot(kwargs["daily_times"][::-1], kwargs["comments"], color="gold")

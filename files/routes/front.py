...
def notifications(v):
    ...
    if messages:
        if v.admin_level == 6:
            ...
            comments = (
                g.db.query(Comment)
                .filter(or_(Comment.author_id == v.id, Comment.sentto == v.id))
                .filter(Comment.parent_submission == None)
                .order_by(Comment.created_utc.desc())
                .offset(25 * (page - 1))
                .limit(26)
    ...
    for c in comments:
        ...
        if (
            c.parent_submission
            and c.parent_comment
            and c.parent_comment.author_id == v.id
        ):
...
def frontlist(
    ...
    posts = (
        ...
        .options(lazyload("*"))
        .filter_by(is_banned=False, stickied=False, private=False)
    ...
    if v and v.hidevotedon:
        posts = [x for x in posts if x.voted == 0]
    ...
    if random.random() < 0.02:
        for post in posts:
            if post.author and post.author.shadowbanned:
                ...
                post.upvotes = (
                    g.db.query(Vote)
                    .filter_by(submission_id=post.id, vote_type=1)
                    .count()
                )
...
def front_all(v):
    ...
    if request.headers.get("Authorization"):
        return {"data": [x.json for x in posts], "next_exists": next_exists}
    else:
        return render_template(
            "home.html",
            v=v,
...
def changeloglist(v=None, sort="new", page=1, t="all", **kwargs):
    ...
    posts = posts.filter(
        SubmissionAux.title.ilike(f"%[changelog]%", User.admin_level == 6)
    )
...
def random_post(v):
    ...
    x = g.db.query(Submission).filter(
        Submission.deleted_utc == 0, Submission.is_banned == False
    )

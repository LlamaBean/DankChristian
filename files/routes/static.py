...
def patrons(v):
    ...
    users = [
        ...
        .filter(User.patron > 0)
        .order_by(User.patron.desc())
        .all()
    ]
...
def log(v):
    ...
    return render_template(
        "log.html", v=v, actions=actions, next_exists=next_exists, page=page
    )
...
def log_item(id, v):
    ...
    return render_template(
        "log.html", v=v, actions=[action], next_exists=False, page=1, action=action
    )
...
def submit_contact(v):
    message = (
        f'This message has been sent automatically to all admins via https://{site}/contact, user email is "{v.email}"\n\nMessage:\n\n'
...
def titles(v):
    ...
    return render_template("/titles.html", v=v, titles=titles)
...
def banned(v):
    ...
    users = [
        x
        for x in g.db.query(User).filter(User.is_banned > 0, User.unban_utc == 0).all()
    ]
...
def brave():
    with open(".well-known/brave-rewards-verification.txt", "r") as f:
        return Response(f.read(), mimetype="text/plain")


@app.get("/.well-known/assetlinks.json")
def googleplayapp():
    with open(".well-known/assetlinks.json", "r") as f:
        return Response(f.read(), mimetype="application/json")


@app.route("/service-worker.js")
def serviceworker():
    with open(".well-known/service-worker.js", "r") as f:
        ...
        if len(header_splitted) == 2:
            ...
            if hmac.compare_digest(req_sign, computed_sign):
                threading.Thread(
                    target=lambda: [
                        time.sleep(2),
                        subprocess.call(["sh", "/vidya.cafe/vidya.cafe/pull.sh"]),
                    ]
                ).start()
    return "OK", 200

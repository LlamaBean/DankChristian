...
def authorize(v):
    ...
    new_auth = ClientAuth(
        oauth_client=application.id, user_id=v.id, access_token=access_token
    )
...
def request_api_keys(v):
    ...
    send_admin(
        1,
        f"@{v.username} has requested API keys for `{request.form.get('name')}`. You can approve or deny the request [here](/admin/apps).",
    )
...
def admin_app_approve(v, aid):
    ...
    access_token = secrets.token_urlsafe(128)[:128]
    new_auth = ClientAuth(oauth_client=app.id, user_id=v.id, access_token=access_token)
    ...
    send_notification(
        1,
        v,
        f"Your application `{app.app_name}` has been approved. Here's your access token: `{access_token}`\nPlease check the guide [here](/api) if you don't know what to do next.",
    )
...
def admin_app_revoke(v, aid):
    ...
    g.db.flush()
    send_notification(
        1, app.author, f"Your application `{app.app_name}` has been revoked."
    )
...
def admin_app_reject(v, aid):
    ...
    g.db.flush()
    send_notification(
        1, app.author, f"Your application `{app.app_name}` has been rejected."
    )
...
def admin_app_id(v, aid):
    ...
    oauth = (
        g.db.query(OauthApp)
        .options(joinedload(OauthApp.author))
        .filter_by(id=aid)
        .first()
    )
    ...
    return render_template(
        "admin/app.html", v=v, app=oauth, listing=posts, next_exists=next_exists
    )
